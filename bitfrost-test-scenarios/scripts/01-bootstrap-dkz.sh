...
log "step 1: bifrost identity keys (fixture-compatible seeds 11..0N) + cold keys"
mkdir -p keys
for i in 1 2 3 4; do
...
log "step 5: wait for stake activation (~2 short epochs), then the active_stake gate"
sleep 150
for i in 1 2 3 4; do
  stake=$(curl -sf "$STORE_API/pools/${POOL_IDS[$i]}" |
    python3 -c 'import json,sys; print(json.load(sys.stdin).get("active_stake") or 0)' || echo 0)
  [ "$stake" != "0" ] && [ -n "$stake" ] ||
    die "pool ${POOL_IDS[$i]} has no active_stake — yaci-store isn't serving the snapshot (the run-dkz ⏳ gate); wait longer or check $STORE_API/pools/${POOL_IDS[$i]}"
done
...
# show-roster needs the rendered (registry-aware) config — render first.
for i in 1 2 3 4; do
  sed -e "s|@REGISTRY_BOOTSTRAP@|$BOOT_REF|" \
    -e "s|@TREASURY_INFO_ASSET_NAME@|$K1_NAME|" \
