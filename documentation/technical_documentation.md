...
*Note:* Using Secp256k1 for Bifrost identity keeps the entire protocol on a single curve. Each SPO's signing share `s_i` and the group public key `Y` are derived during DKG (see Distributed Key Generation section).
...
- **Value**: Bifrost Membership Token (with `TokenName = pool_id`)
- **Datum**:
```json
...
##### 3.3 Security Deposit UTxO

Each registered SPO must maintain a security deposit UTxO at the deposit script address:

- **Value**: Minimum deposit amount in ADA (protocol parameter).
- **Datum**:
```json
{ pool_id       :: ByteArray
, bifrost_id_pk :: ByteArray
...
```
- **Spending Condition**: The deposit can be spent by either:
  1. The SPO (via `bifrost_id_sk` Secp256k1 signature) — for voluntary withdrawal after leaving the protocol.
  2. The current roster (via FROST group signature) — for slashing misbehaving participants.

The deposit script uses `verifySchnorrSecp256k1Signature` for both conditions.

##### 3.4 Registry State UTxO (Patricia Merkle Tree)
...
4. **Outputs**:
   - Membership UTxO at membership script address with:
     - Bifrost Membership Token
     - Datum containing `bifrost_id_pk` and `bifrost_url`
   - Security Deposit UTxO at deposit script address with:
     - Minimum deposit amount in ADA
     - Datum containing `pool_id` and `bifrost_id_pk`
...
7. **Security deposit**: verifies a deposit UTxO with sufficient ADA is created at the deposit script address.

#### 7. Revocation

An SPO can revoke their Bifrost registration by burning their Membership Token.

##### 7.1 Revocation Message
...
##### 7.2 Revocation Transaction

The revocation transaction:

1. **Redeemer**: contains `cold_vkey`, `cold_sig`, and `pmt_membership_proof`.
2. Burns the Bifrost Membership Token.
3. Consumes the Registry State UTxO and outputs it with:
   - New `pmt_root` after removing `pool_id` from the tree.

##### 7.3 On-Chain Verification

The burn policy verifies:
...
4. **PMT membership proof**: verifies `pool_id` exists in the tree.
5. **PMT deletion proof**: verifies new `pmt_root` is the valid result of removing `pool_id` from the old tree.

This allows the SPO to re-register with a new Bifrost identity if needed.
...
If any participant `P_m` misbehaves (invalid proof of knowledge in Round 1, or invalid share in Round 2), the DKG must be restarted with `P_m` excluded and their security deposit slashed.

##### 9.1 Slashing Transaction

The **current roster** (from the previous epoch) constructs and submits a Cardano transaction that spends the security deposit UTxOs of misbehaving participants:

1. **Inputs**: Security Deposit UTxO(s) of misbehaving SPO(s).
2. **Redeemer**: FROST group signature over the slashing message.
3. **Outputs**: Slashed funds sent to the treasury or burned.

The deposit script verifies:
- `verifySchnorrSecp256k1Signature(current_roster_pk, slashing_message, frost_signature)` — proves the current roster authorized the slashing.
...
**Note**: Exclusion is per-DKG only. The PMT remains unchanged; excluded SPOs retain their Bifrost Membership Tokens but must post a new security deposit before participating in future DKGs.
